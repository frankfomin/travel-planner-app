...
import { z } from "zod";
import { SaveCityPlaceId } from "@/lib/actions/saveCityPlaceId";
import { pageIndex } from "@/context/message";

export default function CitySearch() {
  const [value, setValue] = useState("");
  const [delayedValue, setDelayedValue] = useState("");
  const { setIndex, index, setPrevIndex } = pageIndex();
            ...
            {data?.map((city: AutocompleteResult, i: number) => (
              <button
                formAction={async () => {
                  ...
                  const result = schema.safeParse(cityData);
                  ...
                  const res = await SaveCityPlaceId(result.data);
